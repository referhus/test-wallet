...
export default createStore({
  ...
  mutations: {
    setResults(state) {
      const conditions = (x) => {
        // ищем по операции
        const operations = state.filters[1].active.id !== 'all' ? x.operation.id.includes(state.filters[1].active.id) : x
        // ищем по статусу
        const status = state.filters[2].active.id !== 'all' ? x.status.id.includes(state.filters[2].active.id) : x

        const time = state.filters[0].active.id !== 'all' ? setTimeFilter(x, state.filters[0].active.id) : x
      ...
      function setTimeFilter(x, i) {
        ...
        switch (i) {
          case 'day': 
            return dateFormat(x.date).date.getFullYear() == date.getFullYear() && 
